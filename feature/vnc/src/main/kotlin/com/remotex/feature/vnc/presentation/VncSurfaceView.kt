...
class VncSurfaceView(context: Context) : View(context) {
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.actionMasked) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                val dx = x - lastX
                val dy = y - lastY
                if (abs(dx) > 2 || abs(dy) > 2) gestureMoved = true
                if (!scaleDetector.isInProgress) {
                    when {
                        inputMode == VncInputMode.DIRECT_TOUCH && event.pointerCount == 1 -> {
                            mapDirect(event.x, event.y)?.let { (rx, ry) ->
                                pointerX = rx; pointerY = ry
                                onInput(VncInputEvent.Pointer(rx, ry, if (dragging) 1 else 0))
                            }
                        }
                        event.pointerCount == 1 -> {
                            pointerX = (pointerX + dx * 1.25f).toInt().coerceIn(0, remoteWidth - 1)
                            pointerY = (pointerY + dy * 1.25f).toInt().coerceIn(0, remoteHeight - 1)
                            onInput(VncInputEvent.Pointer(pointerX, pointerY, if (dragging) 1 else 0))
                        }
                        event.pointerCount == 2 -> {
                            scrollAccumulatorX += dx
                            scrollAccumulatorY += dy
                            val sx = (scrollAccumulatorX / 48f).toInt()
                            val sy = (scrollAccumulatorY / 48f).toInt()
                            if (sx != 0 || sy != 0) {
                                onInput(VncInputEvent.Scroll(sx, sy, pointerX, pointerY))
                                scrollAccumulatorX -= sx * 48f
                                scrollAccumulatorY -= sy * 48f
                            }
                        }
                    }
                }
            ...
            MotionEvent.ACTION_UP -> {
                if (dragging) {
                    ...
                    when (maxPointers) {
                        1 -> click(1)
                        2 -> if (System.currentTimeMillis() - twoFingerStart < 500) click(4)
    ...
    private fun averageX(event: MotionEvent): Float = (0 until event.pointerCount).sumOf { event.getX(it).toDouble() }.toFloat() / event.pointerCount
    private fun averageY(event: MotionEvent): Float = (0 until event.pointerCount).sumOf { event.getY(it).toDouble() }.toFloat() / event.pointerCount
